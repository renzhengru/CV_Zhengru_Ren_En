\newcounter{num}
\setcounter{num}{2} % !!!!!!!!! 1-English CV; 2 - Chinese CV !!!!!!!!!!!
...
\ifnum\value{num}<2 {\linespread{1.1}}  \else{\linespread{1.4}}\fi % 1.0	single spacing; 1.3	one-and-a-half spacing;1.6	double spacing
...
\vspace{-2em}
\section*{\textbf{\ifnum\value{num}<2 {Work experience}  \else{工作经历}\fi}}\vspace{-1.5em}
    \ifnum\value{num}<2 {
    ...
    & Norwegian University of Science and Technology (NTNU) & \\
          ...
          & \multicolumn{2}{l}{- Topic: Onboard decision support and digitization in intelligent marine operations} \\
          & \multicolumn{2}{l}{\quad (Sea state estimation, model auto-tuning, and wave prediction based on vessel responses) }\\
    ...
    2019.8 -- 至今 & 博士后研究员，海洋技术学院，挪威科技大学 & 特隆赫姆, 挪威 \\
          & \multicolumn{2}{l}{Centre for Research-based Innovation of Marine Operations (SFI MOVE)} \\
          & \multicolumn{2}{l}{- 课题: 智能海洋工程舰载决策支持系统（基于船体运动的波浪谱估计及船体模型修正）} \\
          & \\
    2019.4 -- 2019.8 & 研究员, 海洋操作与土木工程学院, 挪威科技大学 & 奥勒松, 挪威 \\
    & \multicolumn{2}{l}{- 课题: 漂浮式风机一体化安装数值模拟与自动控制}
    \end{tabular}%
\end{table}%
    }
    \fi
...
\vspace{-2em}
\section*{\textbf{\ifnum\value{num}<2 {Education}  \else{教育背景}\fi}} \vspace{-1.5em}
\ifnum\value{num}<2 {
    ...
    2008.9 -- 2012.6 & \textbf{B.Eng.} in Ocean Engineering, Dalian University of Technology (DLUT) & Dalian, China \\
          & \multicolumn{2}{l}{- Thesis: The schematic design of a 19000DWT production oil tanker} \\
    \end{tabular}%
}\else{
\begin{table}[htbp]
    \begin{tabular}{R{0.15\columnwidth}L{0.55\columnwidth}L{0.3\columnwidth}}
    2016.1 -- 2019.8 & \textbf{博士}，挪威科技大学 & 特隆赫姆, 挪威 \\
          & \multicolumn{2}{l}{Centre for Research-based Innovation of Marine Operations (SFI MOVE)} \\
          & \multicolumn{2}{l}{Centre for Autonomous Marine Operations and Systems (NTNU AMOS)} \\
          & \multicolumn{2}{l}{专业: 海洋技术} \\
          & \multicolumn{2}{l}{- 论文题目: 固定式及漂浮式海上风机的自动安装及决策支持系统} \\
...
\vspace{-2em}
\section*{\textbf{\ifnum\value{num}<2 {Research interests}  \else{科研兴趣}\fi}}\vspace{-1.5em}
\ifnum\value{num}<2 {
...
智能海洋工程、海上作业、海上风电、数字化技术、数字化孪生、非线性控制理论、传感器融合、系统辨识、海况估计、多智体系统
\end{changemargin}
}\fi
\vspace*{0.1em}

\vspace{-2em}
\section*{\textbf{\ifnum\value{num}<2 {Teaching experience}  \else{教学经历}\fi}}\vspace{-1.5em}
\ifnum\value{num}<2 {
    ...
    \begin{minipage}{\textwidth}
    \textbf{MR8500 - PhD Topics in Marine Control Systems, 挪威科技大学, 2020}\vspace{-1.00em}
    \begin{itemize}[label={}] \setlength\itemsep{0.5em}
...
% \ifnum\value{num}<2 {
%     \vspace{-2em}
%     \section*{\textbf{Teaching plan}}\vspace{-1.5em}
%     I plan to teach the following courses relevant to robotics at both undergraduate and graduate levels:
%     \vspace{-1.20em}
%     % \begin{minipage}{\textwidth}
%     \begin{itemize}[label={*}] \setlength\itemsep{0.0em}
%     \setlength{\itemindent}{2em}
%     \item Modeling and simulation of physical processes,
%     \item Filtering and sensor fusion,
%     \item Linear and nonlinear control theories.
%     \end{itemize}
%     % \end{minipage}
%     \vspace{-1.20em}
%     The theories and applications to mechanical systems (such as robotics, marine vessels, and airplanes) will be introduced. State-of-the-art research outcomes will be involved into the courses and student projects. The courses can be flexibly adjusted according to the department's education plan.
%     \vspace*{0.1em}
% }\else{
% }\fi

\vspace{-2em}
\section*{\textbf{\ifnum\value{num}<2 {Supervision experience}  \else{指导学生}\fi}}\vspace{-1.5em}
\textbf{\ifnum\value{num}<2 {Co-supervision of PhD student at NTNU}  \else{担任副导师指导博士生}\fi}\vspace{-1.00em}
\begin{itemize}[label={}] \setlength\itemsep{0.5em}
...
\vspace{-0.8em}
\textbf{\ifnum\value{num}<2 {Co-supervision of master students at NTNU}  \else{担任副导师指导硕士生}\fi}\vspace{-1.00em}
\begin{itemize}[label={}] \setlength\itemsep{0.5em}
...
\vspace{-2em}
\section*{\textbf{\ifnum\value{num}<2 {Academic experience}  \else{学术经历}\fi}}\vspace{-1.5em}
\textbf{\ifnum\value{num}<2 {Participation research projects}  \else{主要科研项目}\fi}\vspace{-1.00em}
\ifnum\value{num}<2 {
    \begin{itemize}[label={}] \setlength\itemsep{0.5em}
...
\vspace{-0.8em}
\ifnum\value{num}<2 {
    \textbf{Funding/grants}\vspace{-1.00em}
    \begin{itemize}[label={}] \setlength\itemsep{0.5em}
...
\vspace{-2em}
\section*{\textbf{\ifnum\value{num}<2 {Publications}  \else{科研成果}\fi}}\vspace{-1.5em}
%%%%% Journals %%%%%
% 2021
...
\vspace{-2em}
\section*{\textbf{\ifnum\value{num}<2 {Reviewer for international journals and conferences}  \else{国际期刊及会议审稿人}\fi}}\vspace{-1.5em}
\begin{itemize}[label={}] \setlength\itemsep{0.0em}
\item Marine Structures%， 2021
...
\vspace{-2em}
\section*{\textbf{\ifnum\value{num}<2 {Other experience}  \else{其他经历}\fi}}\vspace{-1.5em}
\ifnum\value{num}<2 {
